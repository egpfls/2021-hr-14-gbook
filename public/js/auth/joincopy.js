...
function onBlurId(){
	...
	$.get('/auth/idchk/'+userid).then(function(r) {
		...
		else showMsg($userid, false, '사용할 수 없어요. 다른 아이디를 사용하세요.');
	}).catch(function(err){
		console.log(err);
	});
}
...
function onJoinSubmit(f){
	...
	var success = Array.from($input).every(function(v){
		return $(v).hasClass('success')
	});
...
$('form[name="joinForm"]').find('input').each(function(i){
	if(!$(this).hasClass('form-control')) successChk =false;
})
console.log(successChk);
*/
